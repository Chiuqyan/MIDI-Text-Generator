...
if __name__ == '__main__':
    ...
    # dataset args
    parser.add_argument(
        "--data", type=str, default="data", help="dataset path" # change to local midi data path
    )
    parser.add_argument(
        "--data-val-split",
        type=int,
    ...
    trainer = Trainer(
        ...
        accumulate_grad_batches=opt.acc_grad,    # default=2
        gradient_clip_val=opt.grad_clip,    # 1.0 default
        accelerator="gpu",
        devices=opt.devices,
        max_steps=opt.max_step,    # default=1e6
        benchmark=not opt.disable_benchmark,    # default=False
        val_check_interval=opt.val_step,    # 1600
        log_every_n_steps=1,
        strategy="ddp",
        callbacks=callbacks,
    )
